...
def detectPicture(file_name):
    ...
    img_for_bin = cv2.resize(img, (round(img.shape[1]), round(
        img.shape[0])), interpolation=cv2.INTER_AREA)
    bin_inv = cv2.bitwise_not(img_for_bin)  # flip image colors
    ...
    for i in range(0, len(segment_dict)):
        ...
        segments += subline
        # print(inters_one_line)

    print(f'time to find segments {time.time() - s} s')
