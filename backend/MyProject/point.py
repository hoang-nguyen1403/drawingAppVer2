...
def find_segments(matrix, points_list):
    bin_inv = cv2.bitwise_not(matrix) # flip image colors
    bin_inv = cv2.dilate(bin_inv, np.ones((5,5)))

    segments_list = {}
    count = 0
    lines = itertools.combinations(points_list,2) # create all possible lines
    line_img = np.ones_like(matrix)*255 # white image to draw line markings on
    for line in lines: # loop through each line
        ...
        conj1 = (bin_line/255 + np.zeros_like(bin_inv))
        n_agree = np.sum(conj==2)
        n_wrong = np.sum(conj1==1)
        precision = n_agree/n_wrong
        if precision > .5:
            cv2.line(line_img, start, end, color=[0,200,0], thickness=2)
            segments_list[count] = {"line": [list(start),list(end)], "precision": precision}
            count += 1
...
# segments = find_segments(img, ext)
...
cv2.imshow('marked img', marked_img)
